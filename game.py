...
class SimpleGame:
    ...
    def __init__(
        ...
        Args:
            game_map (Global): The parsed map model.
            map_path (str): The file path to the loaded map.
            simulation_state (bool): Initial state for the
    ...
    def draw_legend(self, screen: pygame.Surface) -> None:
        ...
        overlay: pygame.Surface = pygame.Surface((legend_w, legend_h),
                                                 pygame.SRCALPHA)
        overlay.fill((40, 40, 40, 210))
        screen.blit(overlay, (legend_x, legend_y))
        pygame.draw.rect(screen, (200, 200, 200), (legend_x,
                                                   legend_y,
                                                   legend_w,
                                                   legend_h),
                         2,
        ...
        # Keyboard controls
        title_controls: pygame.Surface = font_title.render("Controls", True, (
            255, 255, 255))
        ...
        for key, desc in controls:
            text: pygame.Surface = font_text.render(f"{key}: {desc}", True, (
                200, 200, 200))
        ...
        # Zones
        title_zones: pygame.Surface = font_title.render("Hub Zones", True, (
            255, 255, 255))
        ...
        for zone_name, color, desc in zones:
            pygame.draw.circle(screen, color, (legend_x + 25,
                                               y_offset + 12), 8)
            text = font_text.render(f"{zone_name} - {desc}", True, (
                200, 200, 200))
            screen.blit(text, (legend_x + 45, y_offset))
            y_offset += 28
    ...
    def draw_tooltip(self, screen: pygame.Surface) -> None:
        ...
        if not hovered_node:
            for conn in self.game_map.glb_connection:
                ...
                if line_len_sq == 0:
                    continue
                num: float = (world_x - x1) * (x2 - x1) + (world_y - y1) * (
                    y2 - y1)
        ...
        tooltip_bg: pygame.Surface = pygame.Surface((tw, th), pygame.SRCALPHA)
...
class GameApp:
    """Main application controller managing states between
    menus and simulations.

    Initializes the Pygame window and handles the high-level transition between
    the 'MENU', 'PARSING', and 'GAME' states.
    """
