...
def print_simulation_output(flight_plan: dict[str, list[str]]) -> None:
    ...
    Outputs each turn of the simulation, listing the drones that moved and their
    new destinations (or connections if traversing a waypoint).

    Args:
        flight_plan (dict[str, list[str]]): The flight plan generated by TrafficController.
    """
    if not flight_plan:
        return
...
class SimpleGame:
    """Manages the main game loop, rendering, and interaction for a loaded simulation.
    ...
    def __init__(
        ...
        Args:
            ...
            simulation_state (bool): Initial state for the background scrolling animation.
        """
    ...
    def draw_legend(self, screen: pygame.Surface) -> None:
        ...
        overlay: pygame.Surface = pygame.Surface((legend_w, legend_h), pygame.SRCALPHA)
        overlay.fill((40, 40, 40, 210))
        screen.blit(overlay, (legend_x, legend_y))
        pygame.draw.rect(screen, (200, 200, 200), (legend_x, legend_y, legend_w, legend_h), 2, border_radius=10)

        y_offset: int = legend_y + 15

        # Keyboard controls
        title_controls: pygame.Surface = font_title.render("Controls", True, (255, 255, 255))
        ...
        for key, desc in controls:
            text: pygame.Surface = font_text.render(f"{key}: {desc}", True, (200, 200, 200))
        ...
        # Zones
        title_zones: pygame.Surface = font_title.render("Hub Zones", True, (255, 255, 255))
        ...
        for zone_name, color, desc in zones:
            pygame.draw.circle(screen, color, (legend_x + 25, y_offset + 12), 8)
            text = font_text.render(f"{zone_name} - {desc}", True, (200, 200, 200))
    ...
    def draw_tooltip(self, screen: pygame.Surface) -> None:
        ...
        if not hovered_node:
            for conn in self.game_map.glb_connection:
                node_a: VisualNode | None = self.renderer.nodes.get(conn.connection_a)
                node_b: VisualNode | None = self.renderer.nodes.get(conn.connection_b)
                ...
                if line_len_sq == 0:
                    continue
                num: float = (world_x - x1) * (x2 - x1) + (world_y - y1) * (y2 - y1)
                t: float = max(0.0, min(1.0, num / line_len_sq))
                proj_x: float = x1 + t * (x2 - x1)
                proj_y: float = y1 + t * (y2 - y1)
                dist_proj: float = math.hypot(world_x - proj_x, world_y - proj_y)
        ...
        for d in self.renderer.drones_sprites:
            ...
            next_h: str = d.flight_plan[min(d.step + 1, len(d.flight_plan) - 1)]
            ...
            if hovered_node and node_obj:
                ...
                if curr != next_h:
                    ...
                    involved: set[str] = set(get_nodes(curr) + get_nodes(next_h))
        ...
        if text3:
            y_offset: int = box_y + 10 + text1.get_height() + text2.get_height()
            screen.blit(text3, (box_x + 10, y_offset))


class GameApp:
    """Main application controller managing states between menus and simulations.
    ...
    def run(self) -> NoReturn:
        ...
        Continuously processes events, updates the active state logic (Menu or Game),
